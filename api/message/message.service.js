...
async function addMessage(message, user) {
  ...
  try {
    ...
    message.correspandents = message.correspandents.map(
      (correspandent) => new ObjectId(correspandent),
    );
    message.isRead = message.correspandents.map((correspandent) => ({
      id: correspandent,
      isRead: false,
    }));
    message.messages = [];
    ...
    socketService.emitToUsers({
      type: "add-message",
      data: message._id,
      userIds: message.correspandents
        .filter((correspandent) => !correspandent.equals(user._id))
...
async function editMessage(msg, user) {
  try {
    ...
    msg.correspandents = msg.correspandents.map(
      (correspandent) => new ObjectId(correspandent._id),
    );
